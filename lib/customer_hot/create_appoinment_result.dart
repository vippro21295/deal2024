...
class CreateAppoinmentResult extends StatefulWidget {
  final dynamic objectCus;
  final String userName;
  final dynamic lastAppointmentResult;
  const CreateAppoinmentResult(
      {super.key,
      required this.userName,
      required this.objectCus,
      this.lastAppointmentResult});

  @override
  State<CreateAppoinmentResult> createState() => _CreateAppoinmentResultState();
}

class _CreateAppoinmentResultState extends State<CreateAppoinmentResult> {
  ...
  Future<void> getAppointmentResult() async {
    var dataLast = widget.lastAppointmentResult;
    // chua coc = 5303
    if (dataLast["StatusDeposit"] == "5303") {
      isCheckChuaCoc = true;
    } else {
      isCheckChuaCoc = false;
    }
    ...
    if (response.statusCode == 200) {
      ...
      var meetingRe = dataLast["MeetingResult"].toString();
      var lst = [];
      if (meetingRe.contains(";")) {
        lst = meetingRe.split(";");
      } else {
        lst = meetingRe.split(",");
      }
      ...
      for (var eleR in lst) {
        for (var eleData in data) {
          if (eleData["Code"] == eleR) {
            eleData["isChecked"] = true;
          }
          if (eleData["Code"] == 'KH_DEN_CH_TT') {
            isGoToBranch = true;
          }
        }
      }
  ...
  Future<dynamic> addAppointmentResult() async {
    ...
    if (validateSubmit()) {
      ...
      var uri = Uri.parse('${urlApi}createAppointMentResult');
      try {
        var response =
  ...
  bool validateSubmit() {
    ...
    if (errors.isNotEmpty) {
      ...
      appresult["AppointmentId"] =
          widget.lastAppointmentResult["AppointmentId"];
      ...
      if (isCheckChuaCoc) {
        ...
        appresult["StatusDeposit"] = 5303;
      } else {
        widget.objectCus["statusLevel"] = 5302;
        appresult["StatusDeposit"] = 5302;
      }
